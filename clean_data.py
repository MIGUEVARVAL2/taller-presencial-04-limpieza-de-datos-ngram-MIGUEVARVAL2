...
def generate_cleaned_column(df):
    ...
    fingerprint= df.groupby("fingerprint").first().reset_index()
    # 3.  Cree un diccionario con 'fingerprint' como clave y 'text' como valor
    fingerprint = fingerprint.set_index("fingerprint")["text"].to_dict()
    # 4. Cree la columna 'cleaned' usando el diccionario
    df["cleaned"] = df["fingerprint"].map(fingerprint)
    return df
